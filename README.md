...
1. **模块化设计**：使用类来组织问题、疾病模型和评估系统，便于扩展和维护。
2. **灵活性**：可以轻松添加新的疾病评估模型，只需定义相应的问题和评分标准。
3. **问题流程控制**：支持根据用户的回答动态显示后续问题，避免无关问题。
4. **权重系统**：不同问题可以设置不同的权重，反映其在疾病诊断中的重要性。
5. **风险分级**：根据总分将风险分为多个级别，并提供相应的建议。

目前系统中包含了COPD和哮喘两种疾病的评估模型。每个模型都有一系列针对性的问题，用于评估用户患该疾病的可能性。你可以通过扩展`create_xxx_model`函数来添加更多疾病的评估模型，或者修改现有模型的问题和评分标准。

使用时，只需运行程序，选择要评估的疾病，然后按照提示回答问题即可获得风险评估# 1236547
copd
